...
<div class="space-y-6 max-w-4xl">
  ...
  <!-- Genre Workflow -->
  <div>
    <p class="text-xs uppercase tracking-widest text-secondary mb-2 font-semibold">{i18n.t('help.genreWorkflow')}</p>
    <!-- Genre tabs -->
    <div class="flex flex-wrap gap-1 mb-3">
      {#each GENRE_WORKFLOWS as g}
        <button
          class="px-2.5 py-1 rounded text-xs font-bold uppercase tracking-wider border transition-colors
            {activeGenre === g.id
              ? 'bg-primary border-primary text-on-primary'
              : 'border-outline-variant text-secondary hover:bg-surface-container'}"
          onclick={() => activeGenre = g.id}
        >{i18n.t(g.nameKey)}</button>
      {/each}
    </div>
    ...
    <div class="border border-outline-variant rounded bg-surface-container-lowest p-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <h3 class="font-bold uppercase tracking-wide text-sm">{i18n.t(selectedGenre.nameKey)}</h3>
      </div>
      <div class="mb-2">
        <p class="text-xs uppercase tracking-widest text-secondary mb-1 font-semibold">{i18n.t('help.structure')}</p>
        <p class="text-xs text-on-surface bg-surface-container px-3 py-2 rounded font-mono">{i18n.t(selectedGenre.structureKey)}</p>
      </div>
      <div class="mb-2">
        <p class="text-xs uppercase tracking-widest text-secondary mb-1 font-semibold">{i18n.t('help.keyFeatures')}</p>
        <div class="flex flex-wrap gap-1.5">
          {#each selectedGenre.highlightKeys as hk}
            <span class="px-2 py-0.5 text-xs rounded border border-primary text-primary bg-primary/5 font-medium">{i18n.t(hk)}</span>
          {/each}
        </div>
      </div>
      <p class="text-xs text-secondary italic">💡 {i18n.t(selectedGenre.tipKey)}</p>
    </div>
  </div>

  <!-- Skill Reference -->
  <div>
    <p class="text-xs uppercase tracking-widest text-secondary mb-2 font-semibold">{i18n.t('help.skills')}</p>
  ...
  <!-- Agent Reference -->
  <div>
    <p class="text-xs uppercase tracking-widest text-secondary mb-2 font-semibold">{i18n.t('help.agents')}</p>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {#each AGENT_DEFS as agent}
        <div class="flex items-center gap-3 p-3 rounded border border-outline-variant bg-surface-container-lowest">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-secondary mt-0.5">{i18n.t('agent.' + agent.id) || ''}</p>
          </div>
          <span class="text-xs uppercase tracking-wider text-on-surface-variant border border-outline-variant px-2 py-0.5 rounded shrink-0">{agent.genre}</span>
        </div>
      {/each}
    </div>
  </div>
  ...
  <!-- Pipeline Overview -->
  <div>
    <p class="text-xs uppercase tracking-widest text-secondary mb-2 font-semibold">{i18n.t('help.pipelineOverview')}</p>
    <div class="border border-outline-variant rounded bg-surface-container-lowest p-4">
      ...
      <p class="text-xs text-on-surface-variant mt-3">{i18n.t('help.pipelineTip')}</p>
    </div>
  </div>

  <!-- Quick Start -->
  <div>
    <p class="text-xs uppercase tracking-widest text-secondary mb-2 font-semibold">{i18n.t('help.quickStart')}</p>
    <div class="border border-outline-variant rounded bg-surface-container-lowest overflow-hidden">
      {#each [
        ...
        >
          <span class="w-6 h-6 rounded-full bg-primary text-on-primary text-xs flex items-center justify-center font-bold shrink-0">{i + 1}</span>
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-secondary">{i18n.t(step.key)}</p>
          </div>
        </button>
      {/each}
    </div>
